...
# this function generates lists for signal, messages and initial values
  ...
  if CP.radarOffCan:
    ...
    if CP.carFingerprint not in (CAR.CIVIC_BOSCH, CAR.CRV_HYBRID, CAR.INSIGHT):
...
class CarState(object):
  ...
  def update(self, cp, cp_cam):
    ...
    if self.CP.carFingerprint in (CAR.INSIGHT): # TODO: find wheels moving bit in dbc
      self.standstill = cp.vl["ENGINE_DATA"]['XMISSION_SPEED'] < 0.1
      self.door_all_closed = not cp.vl["SCM_FEEDBACK"]['DRIVERS_DOOR_OPEN']
      self.leadDistance = cp.vl["RADAR_HUD"]['LEAD_DISTANCE']
    ...
    if self.CP.carFingerprint in (CAR.CIVIC, CAR.ODYSSEY, CAR.CRV_5G, CAR.CIVIC_BOSCH, CAR.CRV_HYBRID, CAR.INSIGHT):
    ...
    if self.CP.radarOffCan:
      ...
      if self.CP.carFingerprint in (CAR.CIVIC_BOSCH,  CAR.CRV_HYBRID, CAR.INSIGHT):
        self.brake_switch = cp.vl["POWERTRAIN_DATA"]['BRAKE_SWITCH']
